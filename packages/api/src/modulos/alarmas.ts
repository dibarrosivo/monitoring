  ...
  app.get('/alarmas', async (request) => {
    ...
    const filas = await db
      .select({
        ...
        enVerificacionHasta: alarma.enVerificacionHasta,
    ...
    const [contexto] = await db
      .select({
        panel: {
          ...
          enPruebaHasta: panel.enPruebaHasta,
          enPruebaMotivo: panel.enPruebaMotivo,
        },
        sitio: {
          id: sitio.id,
          nombre: sitio.nombre,
  ...
  app.get('/paneles/estado', async () => {
    const paneles = await db
      .select({
        ...
        enPruebaHasta: panel.enPruebaHasta,
        enPruebaMotivo: panel.enPruebaMotivo,
        ventanaCancelacionSeg: panel.ventanaCancelacionSeg,
