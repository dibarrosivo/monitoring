...
/**
 * "hace 2 min", "hace 3 h 12 min": el tiempo sin atender es el dato operativo
 * clave. Con `grueso` (app del cliente) no se cuentan segundos ni restos:
 * "recién", "hace 3 h".
 */
export function transcurrido(iso: string, ahora: number = Date.now(), opciones: { grueso?: boolean } = {}): string {
  const segundos = Math.max(0, Math.floor((ahora - new Date(iso).getTime()) / 1000));
  if (segundos < 60) return opciones.grueso ? 'recién' : `hace ${segundos} s`;
  const minutos = Math.floor(segundos / 60);
  if (minutos < 60) return `hace ${minutos} min`;
  const horas = Math.floor(minutos / 60);
  if (horas < 24) return opciones.grueso ? `hace ${horas} h` : `hace ${horas} h ${minutos % 60} min`;
  const dias = Math.floor(horas / 24);
  return opciones.grueso ? `hace ${dias} d` : `hace ${dias} d ${horas % 24} h`;
}
